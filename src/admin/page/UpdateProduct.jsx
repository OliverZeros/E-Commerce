...
const UpdateProduct = () => {
  ...
  const [slot, setSlot] = useState(existingProduct?.slot || 0);
  const [rating, setRating] = useState(existingProduct?.rating || 5);
  const [description, setDescription] = useState(existingProduct?.description || "");
  const [image, setImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(
    Array.isArray(existingProduct?.imageUrl)
      ? existingProduct?.imageUrl[0]
      : existingProduct?.imageUrl || null,
  );

  const [productType, setProductType] = useState({
    category: existingProduct?.productType?.category || "Sofa",
    color: existingProduct?.productType?.color || "Light",
    size: existingProduct?.productType?.size || "Small",
    model: existingProduct?.productType?.model || "Modern",
  });

  const [loading, setLoading] = useState(!existingProduct);
  const [submitting, setSubmitting] = useState(false);

  // Fallback fetch if not in redux state
  useEffect(() => {
      ...
      const fetchDetail = async () => {
        try {
          ...
          if (p) {
            ...
            setImagePreview(Array.isArray(p.imageUrl) ? p.imageUrl[0] : p.imageUrl);
            setProductType({
              category: p.productType?.category || "Sofa",
              color: p.productType?.color || "Light",
              size: p.productType?.size || "Small",
              model: p.productType?.model || "Modern",
            });
          }
        } catch (error) {
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <div className="d-flex align-items-center gap-2 text-muted small mb-1">
              ...
              <span>/</span>
              <span className="text-dark fw-semibold">Edit Product</span>
            </div>
            <h2 className="fw-bold mb-0 text-dark">Update Furniture Product</h2>
          </div>
              ...
              <div className="product__studio-card h-100">
                ...
                <FormGroup className="mb-0">
                  ...
                  <Input
                    ...
                    placeholder="Product description, dimensions, specifications..."
                    required
                  />
                </FormGroup>
              </div>
            ...
            <Col lg="5" md="12">
              <div className="product__studio-card mb-4">
                ...
                <div className="text-center">
                  ...
                  >
                    <i className="ri-upload-2-line me-1"></i> Replace Product Image
                  </label>
                </div>
              </div>
